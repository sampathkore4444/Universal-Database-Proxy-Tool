...
func (e *ShardingEngine) AddShard(shard *DataShard) {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *ShardingEngine) GetShard(id int) (*DataShard, bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func (e *ShardingEngine) ListShards() []*DataShard {
	e.mu.RLock()
	defer e.mu.RUnlock()

	shards := make([]*DataShard, 0, len(e.shards))
...
func (s *DataShard) GetStats() map[string]interface{} {
	s.mu.RLock()
	defer s.mu.RUnlock()
