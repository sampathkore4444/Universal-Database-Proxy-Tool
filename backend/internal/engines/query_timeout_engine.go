...
type QueryTimeoutEngine struct {
	...
	activeQueries map[string]*types.QueryContext
	mu            sync.RWMutex
}
...
func NewQueryTimeoutEngine(config *TimeoutConfig) *QueryTimeoutEngine {
	...
	return &QueryTimeoutEngine{
		...
		config:        config,
		activeQueries: make(map[string]*types.QueryContext),
	}
}
...
func (e *QueryTimeoutEngine) GetActiveQueries() []*types.QueryContext {
	e.mu.RLock()
	defer e.mu.RUnlock()

	queries := make([]*types.QueryContext, 0, len(e.activeQueries))
